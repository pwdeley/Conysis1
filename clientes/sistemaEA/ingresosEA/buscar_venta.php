    ...
    <table>
        ...
        while($sistemas = mysqli_fetch_row($sql_registe)){
            ...
            <tr>
                ...
                <td class="textright"><?php
                    if ($sistemas[11] == 'Tarjeta'){
                    echo number_format((($sistemas[7] * $sistemas[8])+($sistemas[9] * $sistemas[10]))*0.12*0.7,2,',','.')
                    ?>
                </td>
            <?php
            }
            ?>
                ...
                <td class="textright"><?php
                    if($sistemas[5] == '45'){
                        echo number_format( (($sistemas[7] * $sistemas[8])+($sistemas[9] * $sistemas[10]))*0.12,2,',','.');
                    }else{
                        echo number_format( (($sistemas[7] * $sistemas[8])+($sistemas[9] * $sistemas[10]))*0.12,2,',','.'); } ?></td>
                
                <td><img src=" <?php echo $sistemas[12] ?>" width="50" height="50" alt="" srcset=""></td>
        ...
        $suma3 = mysqli_query($conection,"SELECT SUM(precio1*cantidad1) as miva1 FROM facturaV WHERE fechaemis BETWEEN '$fecha_de' AND '$fecha_a'");
        $row3 = mysqli_fetch_array($suma3,MYSQLI_ASSOC);
        $suma4 = mysqli_query($conection,"SELECT SUM(precio2*cantidad2) as miva2 FROM facturaV WHERE fechaemis BETWEEN '$fecha_de' AND '$fecha_a'");
        $row4 = mysqli_fetch_array($suma4,MYSQLI_ASSOC);
        $suma5 = mysqli_query($conection,"SELECT SUM(precio1*cantidad1*0.02) as mtotal5 FROM facturaV WHERE fechaemis BETWEEN '$fecha_de' AND '$fecha_a' AND formacobro != 'Efectivo' AND formacobro != 'Transferen'");
        $row5 = mysqli_fetch_array($suma5,MYSQLI_ASSOC);
        $suma6 = mysqli_query($conection,"SELECT SUM(precio2*cantidad2*0.02) as mtotal6 FROM facturaV WHERE fechaemis BETWEEN '$fecha_de' AND '$fecha_a' AND formacobro != 'Efectivo' AND formacobro != 'Transferen'");
        $row6 = mysqli_fetch_array($suma6,MYSQLI_ASSOC);
        $suma7 = mysqli_query($conection,"SELECT SUM(precio1*cantidad1*0.12*0.7) as mtotal7 FROM facturaV WHERE fechaemis BETWEEN '$fecha_de' AND '$fecha_a' AND formacobro != 'Efectivo' AND formacobro != 'Transferen'");
        $row7 = mysqli_fetch_array($suma7,MYSQLI_ASSOC);
        $suma8 = mysqli_query($conection,"SELECT SUM(precio2*cantidad2*0.12*0.7) as mtotal8 FROM facturaV WHERE fechaemis BETWEEN '$fecha_de' AND '$fecha_a' AND formacobro != 'Efectivo' AND formacobro != 'Transferen'");
