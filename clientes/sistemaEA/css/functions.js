$(document).ready(function(){
    ...
    // Buscar Producto 1 por código:
    $('#txt_cod_producto1').keyup(function(e){
        e.preventDefault();
               ...
               if(response != 'error')
                {
                    ...
                    $('#txt_cant_producto1').val('0');
                    $('#txt_precio1').val('0');
                    //$('#txt_precio').html(info.precio);
                    $('#txt_precio_total1').html(info.precio1);

                    //Activar cantidad
                    $('#txt_cant_producto1').removeAttr('disabled');

                    //Activar PRECIO UNITARIO
                    $('#txt_precio1').removeAttr('disabled');
...
// Buscar Producto 2 por código:
$('#txt_cod_producto2').keyup(function(e){
    e.preventDefault();
    ...
    $.ajax({
        ...
        success: function(response)
        {
           if(response != 'error')
            {
                var info = JSON.parse(response);
                $('#txt_descripcion2').html(info.descripcion);
                $('#txt_existencia2').html(info.existencia);
                $('#txt_cant_producto2').val('0');
                $('#txt_precio2').val('0');
                //$('#txt_precio').html(info.precio);
                $('#txt_precio_total2').html(info.precio2);

                //Activar cantidad
                $('#txt_cant_producto2').removeAttr('disabled');

                //Activar PRECIO UNITARIO
                $('#txt_precio2').removeAttr('disabled');

                //Mostrar botón agregar
                $('#add_product_venta').slideDown();
            }else{
                $('#txt_descripcion2').html('-');
                $('#txt_existencia2').html('-');
                $('#txt_cant_producto2').val('0');
                $('#txt_precio2').val('0');
                //$('#txt_precio').html('0.00');
                ...
                //Bloquear Cantidad
                $('#txt_cant_producto2').attr('disabled','disabled');
...
// Buscar Proveedor por RUC:
        ...
        var action = 'searchProveedor';
                ...
                success: function(response)
                {
                    ...
                    if(response == 0)
                    {
                        ...
                        var data = $.parseJSON(response);
                        $('#nombre_proveedor').val(data.nombreprovee);
    ...
    */
    ...
    $('#txt_cant_producto').keyup(function(e){
        ...
        var precio_total1 = $(this).val() * $('#txt_precio1').val();
        var existencia = parseInt($('#txt_existencia').html());
        $('#txt_precio_total1').html(precio_total1);
        ...
        $('#add_product_venta').click(function(e){
            ...
            {
                var codproducto1 = $('#txt_cod_producto1').val();
                var cantidad1    = $('#txt_cant_producto1').val();
                var precio1      = $('#txt_precio1').val();
            ...
            success: function(response){
                if(response != 'error'){
                    ...
                    $('.bodyModal').html('<form action="" method="post" name="form_anular_factura" id="form_anular_factura" onsubmit="event.preventDefault(); anularFactura();">'+
                        '<h1><a style="font-size: 45pt;">&#128230;</a><br>Anular Factura</h1><br>'+
                        '<p>¿Está seguro de ANULAR el siguiente factura?</p>'+

                        '<p><strong>No. '+info.nofactura+'</strong></p>'+
                        '<p><strong>Valor. $.'+info.precio1+'</strong></p>'+
                        '<p><strong>No. '+info.fecha+'</strong></p>'+
